...
- **Slash commands**: 6 commands surfaced via `.claude-plugin/commands/` for the `/plugin` Discover screen (v2.1.145+) — `/scaffold`, `/audit-release`, `/audit-cd`, `/audit-security`, `/proof-report`, `/graduate`. Each command wraps an MCP tool / workflow with explicit positioning (e.g., `/audit-security` notes complementarity with Anthropic's in-session `claude-security-guidance` plugin).
